...
`<user-queue>` (AI → user) and `<machine-queue>` (user → AI) mirror in structure: each a checkable `.md` one party writes and the other reads. The checkbox semantics and the proposal shape a producer writes are defined in the action-agent SKILL; only a judgment call reaches `<user-queue>`, never a routine fixable violation a script handles.
...
- **A question asked in chat that goes unanswered lands here** before the session ends, in the proposal shape — an in-chat question must not evaporate with its session.
