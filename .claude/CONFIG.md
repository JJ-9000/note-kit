...
| note-kit-anonymize        | no              | —                                          | identity gate at any outward boundary — wraps `verify_publish_anonymity.py` on the staged bytes or named files; a hit holds the outward action until the user clears it, clean content passes silently |
...
The one home for missing-field inference (queue-ruled 2026-08-03; the janitor and filing SKILLs cite this section rather than restating it). A consumer walks its field's ladder top-down and takes the first rung that yields one confident answer, applying the value with the `inferred` tag; a rung holding two or more meaningfully different answers stops the walk and routes the call to the consumer's own escalation (the janitor's queue proposal, filing's hold disposition). The filing-agent's re-inference of a `parent:` or `project:` that resolves nowhere walks the same parent/project rows as a missing field.
...
The harness allowlist is **defined here and mirrored** into `settings.local.json` by `sync_config` (merge; hand-added entries are preserved). Edit this table, then re-run sync. Path tokens in a rule (`<user-home>`, `<vault-root>`) resolve to the install's literal paths at mirror time, and the stale token-form twin of a mirrored rule drops when its resolved form lands. A global `defaultMode` in `<user-home>/.claude/settings.json` outranks this allowlist — when one is set, the mode is the live gate and this table documents intent as the fallback posture; the § Holds and approvals guardrail floor binds behaviorally regardless.
...
1. **One mutating agent at a time, vault-wide.** A run takes the lease file `<logs>/run-lease.md` — one line in `run_lease.py`'s canonical shape, **timestamp first**: `<timestamp> | <agent> | <action> | <note>`, written through the `RunLease` helper — and releases it at exit; a second mutating agent finding a fresh lease waits for its next cadence. A stale lease past 2 hours is expired, logged, and taken over by an acquiring run; a read-only launcher finding a non-empty lease it cannot parse defers, since an illegible claim is still a claim and only an acquirer, which logs what it displaces, may take one over.
...
- **Outward publication:** run a store-back to the publication repo or a git push only on an approved item naming it — git author checked first, then `verify_publish_anonymity.py` over the staged bytes (§ Helper-script automation): every identity hit is removed, replaced with its proposed anonymous token, or allowlisted with a recorded reason before the push runs; a clean scan passes silently. A whole-kit release waits while the driving plan holds open, non-deferred checkboxes; flip a checkbox only on an independent read of the artifact.
...
| `scaffold_vault.py` | manual; documented in README.md. Seeds `<machine-queue>` and `<user-queue>` with checkable example items; `--with-ui-plugin <dir>` installs the note-kit-ui Obsidian plugin and merges it into `community-plugins.json`; `--upgrade` also refreshes an installed note-kit-ui plugin + Note-Kit theme (appearance selection kept; repointed only across a theme rename); a non-empty non-kit `--path` asks for confirmation — `--into-existing` pre-confirms, a non-interactive run refuses |
...
| `verify_publish_anonymity.py` | pre-push on any store-back to the publication repo (§ Self-modification), over the staged bytes; `--head`/`--payload` for whole-surface audits, `--paths` for any other outward boundary; every private canary derives at runtime (home path, git identity, machine name, repo tree names, vault and sandbox literals) and self-tests before the run counts; blobs are scanned in UTF-8 and UTF-16 decodings; a hit proposes its anonymous token and exit 2 holds the push |
...
| `agent_gate.py` | invoked by `scheduled_gate_launcher.py` before any model load; read-only — prints `gate: work`/`gate: quiet` from cheap disk signals per agent (queue sweeps, inbox drop/staged fingerprints, corpus fingerprint, per-agent log-staleness override); never writes — the launcher records the fingerprint only when a spawn happens |
| `scheduled_gate_launcher.py` | the four scheduled agents' gated entry point for Windows Task Scheduler (tasks `note-kit-*-agent-gated`, registered 2026-08-03 and held disabled until the headless CLI authenticates — the Desktop-app registrations carry the cadences meanwhile): per-agent lock → read-only lease check (fresh holder defers) → `agent_gate` → a quiet verdict exits silently writing nothing; a work verdict spawns `claude -p` against the deployed SKILL with `--max-budget-usd` and no permission-mode flag (the global default applies), then appends one `gate-launch` line and records the fingerprint. Each spawn's output lands in `<logs>/gate-launcher-<agent>-last.json`; the next run reports a captured error with one `last-launch-errored` line and clears both capture and fingerprint, so gated work re-arms rather than starving behind a failed launch |
