...
- **Supersede a parked approval.** Each run, read every parked `[x]` against the live record its work tracks — the linked plan's executed state, the repo, the project page. When that record already carries the action's outcome — a commit, a shipped version, a plan gate marked done — the action is superseded: archive the queue snapshot, retire the line, and log it `queue-superseded` naming the record. Leave a superseded action retired; reopening or re-speccing it only trails the live work. Retire on the record's own state; an item it has not overtaken keeps its `[x]` and stays parked.
