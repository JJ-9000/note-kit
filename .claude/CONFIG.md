...
- **Hold a true hold, briefly.** Keep in the queue only an item that is indivisible, tied to no project, and risky — a vault-wide refactor that would break agent function. Warn the user once in `<user-queue>` and follow the instruction left there; held a second pass, move it to a `<plan>` in `<inbox>` and clear the line.
- **An un-actionable item migrates.** A `<machine-queue>` item that resolves to no action at all — names no skill, content, target, or directive even after shape recovery — never lingers `[ ]`: the agent removes the line (queue snapshot archived first) and raises one `<user-queue>` clarification quoting the original text verbatim, with a fill-in option to restate the instruction. An understood but live-blocked line elevates to a `<plan>` (above), never holding a `[ ]` in the queue; an unintelligible line has no work to hold a place for, so it migrates to one clarification.
...
- **No silent deferral.** No hold sits silently in the queue — it supersedes or elevates by the rules above.
