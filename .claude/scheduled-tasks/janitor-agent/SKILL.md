...
`JANITOR_VAULT_ROOT` is the vault root. Writes are gated: a bare run is detect-only; `--apply` (or `JANITOR_APPLY=1`) applies the deterministic fixes — including pruning an empty subfolder and an empty index — and refreshes the state snapshot this run consumes. A trustworthy deterministic fix auto-applies silently with one log line and **no janitor row**; the janitor receives only genuine inference and judgment calls. If the script exits non-zero or prints a traceback, stop and report the error.
...
- **Event log:** append one line per resolution to `<logs>/janitor-agent/janitor-agent.md` (CONFIG § Log files), one line per cluster routed to the queue. A run that resolves nothing appends nothing.
