...
Usage
-----
    JANITOR_VAULT_ROOT=/path/to/vault python build_state_index_v003.py \
...
def main() -> None:
    ...
    sections: list[str] = []
    sections.append(f"<!-- Generated by scripts/build_state_index_v003.py at {ts}. Run #{run_number}. -->")
    sections.append("")
    ...
    for record in fm_records:
        ...
        if isinstance(tags_raw, list):
            from config_variables_v002 import levenshtein_le1, CANONICAL_TAG_KEYS
            for tag in tags_raw:
                tag_str = str(tag).strip().lower()
