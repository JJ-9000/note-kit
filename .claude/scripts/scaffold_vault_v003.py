...
Usage:
  python scripts/scaffold_vault_v003.py [--path <dir>] [--clean]
  python scripts/scaffold_vault_v003.py --path <vault>   # in-place ok: kit at the vault root
  python scripts/scaffold_vault_v003.py --path <vault> --with-ui-plugin <plugin-dir>
  python scripts/scaffold_vault_v003.py --upgrade <vault>
"""
...
from config_variables_v002 import FOLDER_ROUTING, SUBFOLDERS, _folder_by_semantic  # noqa: E402
...
def _run_upgrade(vault_root: Path) -> int:
    ...
    sync_script = kit_dir / "scripts" / "sync_config_v003.py"
...
def main() -> None:
    ...
    _SANDBOX_MARKER = (
        ...
        "`scripts/scaffold_vault_v003.py`. Not a real install. Safe to delete.\n\n"
    )
    ...
    if user_queue.exists():
        ...
        user_queue.write_text(
            ...
            "(CONFIG § Queue protocol; proposal shape per the action-agent "
            "SKILL).\n\n"
            "## Example proposals\n\n"
            "Worked examples in the canonical proposal shape — check an option "
            "to see the loop run, or delete them:\n\n"
            "`Welcome-Note.md` (example) — an inbox draft needs a home; choose "
            "where it files.\n\n"
            ...
            "`recipes` (example) — a tag appeared that CONFIG § Tags does not "
            "list; decide its fate.\n\n"
