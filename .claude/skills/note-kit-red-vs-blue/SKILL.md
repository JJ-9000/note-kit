...
| Healthy decoy | a variant with **no** fault, indistinguishable in framing from a faulted one. Decoys measure precision. |
...
| Softball | a match the current instrument was already expected to win: the expected signal lands in a well-covered channel and the solver one-shots it with high confidence. Softballs score as regression checks only and cap at one per batch. |
...
1. Copy the fixture to the sandbox **with every dependency it resolves at runtime**.
...
5. Where available, add **sibling controls** — other healthy subjects of the same kind.

**Output:** `<notes>/calibration-vNNN.md`.

## Phase 1 — Catalogue

Every fault must carry **provenance** (see Definitions) — a real failure someone actually hit. Build it before authoring anything:
...
4. Mark entries the current instrument architecture plausibly cannot reach — candidate documented-hards, kept in the battery anyway.
...
1. One catalogue entry per variant, **one fault per variant** in early batches. Prepare each in the sandbox via the calibrated path; admit it to the battery only when it **reproduces the reported symptom** — re-read the broken site *and* observe the behavior the provenance source described (the grey render, the silent no-output, the wrong count). A change that landed but produces no symptom is not the catalogued fault; rework it or drop it.
2. Batch composition is majority `expected-blind`/`unknown`; at most one `expected-catch` regression check per batch.
...
1. **Raw-layer miss** → a new or extended detector in the instrument. **Lens-layer miss** → a lens fix, leaving the instrument unchanged. **Solver-layer miss** → a framing or report-ordering fix, or accept as solver variance.
...
3. Gates, all three before the version is used in play: **quiet-on-healthy** (nothing new on any control in the corpus) · **catch** (the motivating variant now surfaces) · **budget** (control-output size within the Phase-0 budget).
...
5. A blind spot the architecture cannot reach → **documented-hard**, with its named resolution path, in the Round Index. After the hardening-attempt cap on one blind spot, stop and classify (CONFIG § Loop budget); raise genuine judgment calls to the `<user-queue>`.
...
- **The instrument's newest surface.** Author variants aimed squarely at every detector and code path added since the last batch — faults the new collector should catch but at its edges, inputs that make it false-alarm, and faults its addition may have masked.
...
Climb one rung per quiet batch, and aim each rung at the instrument's **best-performing channels**:
