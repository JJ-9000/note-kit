...
- **External or pasted text** (no vault home): land a new draft in the skill's `<inbox>` container (CONFIG § Skill slugs), the source untouched. A new file is named by its type's pattern (CONFIG § Types).

Frontmatter on the output: keep the source doc's own `type`, `tags`, `date`, and `parent`, then add the `claims-verified` tag, `source: <relative path to the archived original or source doc>`, `status: draft`, and `reviewed: false`. External or pasted text with no frontmatter takes the type the kit infers for it. Phase 5 logs per-run stats to the archive, not the note frontmatter.
...
Hold the claim set in memory for a short pass; for a long doc, persist the partial set in the container's `<notes>/` so a re-invocation continues from it.
...
Pick a verification domain per claim — the knowledge area that defines its primary source: `api-docs` (the vendor's reference), `academic` (the paper itself — DOI, PMID, publisher PDF), `industry-standard` (the RFC/W3C/ISO itself), `news-of-record`, `vendor-blog`, `personal-observation` (the user's own dated observations, valid only for their environment and only when they name the source), `anecdotal` (lowest — forum/social posts, weight only when a critical mass aligns with evidence). When a claim's subject fits none, coin a doc-local domain (e.g. `food-safety`) without asking and name its authority. That a document exists somewhere is not enough; it must be accessible and legible.
...
If a fetched URL is derivative, follow the chain to the primary and re-fetch. Quote only verbatim text from a fetched primary, never a search-result summary. Extract a verbatim quote ≤30 words; before embedding, replace inner `"` with `'`, escape `]` and `)`, strip HTML.
...
**Footnote — VERIFIED / DISPUTED.** One line: verdict tag, an em-dash, then the citation. Single-line only:

```
[^slug]: **VERDICT | PRIORITY | DOMAIN** — [Source title — section](URL-or-path) — "verbatim quote ≤30 words" Accessed YYYY-MM-DD.
```

Keep the footnote lean: verdict tag, citation, quote, and at most one short forward-useful caveat (a scope limit, a value the source omits). Never a process note — what a value used to be, what the run tried, what was corrected, softened, or extended.
...
The admission states the limit in plain prose.

**Limit categories.** The limit an UNRESOLVED claim names comes from the closed vocabulary in CONFIG § Source-outcome categories. Each UNRESOLVED claim also gets a task line: `- [ ] Re-verify: <slug> in [[<stem>]] — <category>: <one-line fix>`. The fix names a concrete path — a publication, an endpoint, an access method, a reframing. A `no-authority` claim is terminal: no re-verify line; its admission is the resolution.

**Voice.** The merged note is read cold, so write any prose you add — recontextualized admissions, the Phase 5 summary — to the resident voice standards (CLAUDE.md § Resident standards). Prefer a comma, colon, or fresh sentence to an em dash where the line reads as well without one. Keep the author's own wording and em dashes; flag a deeper voice issue rather than rewriting it. The footnote citation format keeps its ` — ` separators.
