...
- A user cross-off is a skip, not a completion: the line carries the literal marker `*(item skipped)*` beside its `[x]` (0.4.16); the sweep clears it without executing and logs `machine-queue-skip`. An unmarked `[x]` is the agent's own receipt.
